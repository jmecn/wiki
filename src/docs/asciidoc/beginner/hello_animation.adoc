= jMonkeyEngine 3 教程 (7) - 骨骼动画
:author: (翻译 jmecn)
:revnumber:
:revdate: 2018/01/22 16:04
...
上一篇: <<beginner/hello_material#,材质>>,
下一篇: <<beginner/hello_picking#,鼠标拣选>>
...
.  Create a new key trigger mapping and action. (see: <<beginner/hello_input_system#,Hello Input>>)
...
Now that your character can walk, wouldn't it be cool if it could also pick up things, or aim a weapon at things, or open doors? Time to reveal the secrets of <<beginner/hello_picking#,鼠标拣选>>!
