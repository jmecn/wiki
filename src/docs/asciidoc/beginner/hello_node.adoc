...
. 你创建一些物体，比如玩家，建筑等等。
. 你把这些物体添加到场景里。
. 移动，旋转它们，或给它们调整大小，上色和制作动画。

你将学习场景如何表达3d的世界和rootNode的重要性。 你将学习如何制作简单的物体，如何让他们携带自定义信息（比如生命值）和如何变换它们的坐标，旋转和大小。你将了解场景里两种不同的 "`Spatials（空间物体）`" ：Nodes（节点）和Geometries（几何体）。
 ...
 * 你可以通过物体的父节点旋转，移动和改变这个物体的大小
 * rootNode（根节点） 是特别的：只有被附着在rootNode的节点才会显示在场景中*/
public class HelloNode extends SimpleApplication {
    ...
    public void simpleInitApp() {
        ...
        /** Create a pivot node at (0,0,0) and attach it to the root node 
            在坐标（0,0,0）创建节点，并把它附着在根节点*/
        Node pivot = new Node("pivot");
        rootNode.attachChild(pivot); // put this node in the scene

        /** Attach the two boxes to the *pivot* node. (And transitively to the root node.) 
            把两个方块附着到这个节点上。（过渡性的附着在根节点上）*/
...
编译和运行代码。你应该会看到两个有颜色的和相同旋转角度的方块。
